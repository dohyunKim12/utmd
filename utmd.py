...
def initialize():
    ...
    if not logger.hasHandlers():
        ...
        print(f"Log file: {os.getcwd()}/{os.path.basename(__file__)}.log")

    global TOPIC_NAME, SERVER_IP, KAFKA_PORT, UTM_PORT \
...
def daemonize():
    process = multiprocessing.Process(target=daemon_process, daemon=True)
    process.start()
    print(f"Daemon process started with PID: {process.pid}")
    sys.exit(0)
